...
procedure RegisterNonActiveX(ComponentClasses: array of TComponentClass;
  AxRegType: TActiveXRegType);
begin
  if Assigned(RegisterNonActiveXProc) then
...
begin
  with FList.LockList do
  try
    if (Index < 0) or (Index >= Count) then Error('', Index);
...
begin
  if not (Item is FItemClass) then TList.Error('', 0);
  FItems.Add(Item);
...
begin
  if not TokenSymbolIs(S) then ErrorFmt('', [S]);
end;
...
begin
  ...
  begin
    ...
    if FSourceEnd = FBuffer then Error('');
  end;
...
begin
  if FComponents = nil then TList.Error('', AIndex);
